...
class Stadium22 extends BaseStadium implements StadiumInterface
{
    ...
    public function comments(int $raceNumber, ?string $date = null): array
    {
        ...
        foreach (range(1, 6) as $bracket) {
            ...
            $response['bracket_' . $bracket . '_racer_yesterday_comment_label'] = '前日コメント';
            $response['bracket_' . $bracket . '_racer_yesterday_comment'] = $this->normalize($comments['.box'][$bracket * 2 - 1] ?? '');
        }
